...
namespace FGUI
{
	...
	public:
		...
		// @args: std::shared_ptr<FGUI::CContainer> container = target container
		// bool padding = toggles widget padding on/off (defaults to true)
		CBuilder& AutoSize(std::shared_ptr<FGUI::CContainer> container, int padding = 0) {
			const auto containerSize = container->GetSize();
			if (!container->GetParentWidget()) {
				padding = 1;
			}

			m_pTemporaryWidget->SetSize(containerSize.m_iWidth - padding, containerSize.m_iHeight - padding);
		...
		// @brief: adds a set of tab buttons inside the TabPanel
		// @args: std::vector<std::string> tabs = tab buttons
		CBuilder& Bind(void* data)
		{
			if (m_pTemporaryWidget->GetType() == static_cast<int>(WIDGET_TYPE::CHECKBOX))
			{
				std::reinterpret_pointer_cast<FGUI::CCheckBox>(m_pTemporaryWidget)->Bind(WIDGET_BIND_TYPE::BOOL, data);
			}

			if (m_pTemporaryWidget->GetType() == static_cast<int>(WIDGET_TYPE::SLIDER))
			{
				std::reinterpret_pointer_cast<FGUI::CSlider>(m_pTemporaryWidget)->Bind(WIDGET_BIND_TYPE::FLOAT, data);
			}
		...
		CBuilder& NoBorder()
		{
			if (m_pTemporaryWidget->GetType() == static_cast<int>(WIDGET_TYPE::CONTAINER))
			{
				std::reinterpret_pointer_cast<FGUI::CContainer>(m_pTemporaryWidget)->SetBorder(false);
			}
