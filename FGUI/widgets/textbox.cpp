...
namespace FGUI
{
	...
	void CTextBox::Geometry(FGUI::WIDGET_STATUS status)
	{
		...
		if (m_bIsGettingKey && !m_bIsTextSelected)
		{
			...
			// draw caret
			FGUI::RENDER.Rectangle(arWidgetRegion.m_iLeft + (dmTitleTextSize.m_iWidth + 20) + (strTemporaryTextSize.m_iWidth + 1), arWidgetRegion.m_iTop + (arWidgetRegion.m_iBottom / 2) - (dmTitleTextSize.m_iHeight / 2), 1, dmTitleTextSize.m_iHeight, { 255, 255, 255 });
		}
